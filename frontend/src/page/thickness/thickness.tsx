...
const columnHelper = createColumnHelper<IThickness>();

function Thickness() {
  const { id } = useParams<{ id: string }>();
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [editingThickness, setEditingThickness] = useState<IThickness | null>(null);
  const [thicknessData, setThicknessData] = useState<IThickness[]>([]);
